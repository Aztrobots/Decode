...
public class Auto18CloseG3 extends OpMode {
    ...
    public void loop() {
        ...
        // Se actualiza aquí (antes del switch) para que todos los estados lo vean.
    ...
    // Helpers
    // atEnd(): true solo si el flag de settle ya se armó Y Pedrito reporta fin de path.
    // Usar este método en todos los estados que esperan el fin de un followPath().
    ...
    private void runFSM() {
        switch (state) {
            case PRELOAD:
                if (atEnd()) {
                    // Llegamos a shoot pose — intake off
            ...
            case STACK2:
                if (atEnd()) {
                    follower.holdPoint(
                            new BezierPoint(new Pose(121.734, 60.450, Math.toRadians(-90))),
                            Math.toRadians(-90));
    ...
    private void buildPaths() {
        ...
        toGate1 = follower.pathBuilder()
                .addPath(new BezierLine(SHOOT_POSE, GATE_TAKE1))
        ...
        toGate2 = follower.pathBuilder()
                .addPath(new BezierLine(SHOOT_POSE, GATE_TAKE2))
                .setLinearHeadingInterpolation(Math.toRadians(0), Math.toRadians(40))
                .build();
        ...
        toGate3 = follower.pathBuilder()
                .addPath(new BezierLine(SHOOT_POSE, GATE_TAKE3))
                .setLinearHeadingInterpolation(Math.toRadians(0), Math.toRadians(40))
                .build();
    ...
    private void transition(AutoState newState) {
        ...
        shotFired      = false;
        pathEndArmed   = false;   // reset del settle-guard en cada transición
    }
